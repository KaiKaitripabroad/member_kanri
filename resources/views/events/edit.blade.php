<x-app-layout>
    <div class="py-6 px-4 pb-24">
        <div class="flex justify-between items-center mb-6">
            ...
            <span class="font-bold text-gray-900">イベント編集</span>
            <button type="submit" form="event-form" class="text-indigo-600 font-medium text-sm">保存</button>
        </div>

        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <form id="event-form" action="{{ route('events.update', $event) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <div class="relative">
                                <input type="date" id="event_date" name="event_date" value="{{ old('event_date', $event->event_date?->format('Y-m-d')) }}"
                                       class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-gray-900"
                                       required>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                                  class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-gray-900"
                                  placeholder="イベントの内容や持ち物などを入力...">{{ old('description', $event->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex gap-6">
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="radio" name="attendance_enabled" value="1" {{ old('attendance_enabled', $event->attendance_enabled) ? 'checked' : '' }}
                                       class="rounded-full border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <span class="ml-2 text-sm text-gray-700">有効にする</span>
                            </label>
                            <label class="inline-flex items-center cursor-pointer">
                                <input type="radio" name="attendance_enabled" value="0" {{ !old('attendance_enabled', $event->attendance_enabled) ? 'checked' : '' }}
                                       class="rounded-full border-gray-300 text-indigo-600 focus:ring-indigo-500">
